...
image_path = 'architectural_plan2.jpg'  # Replace with your image path
image = io.imread(image_path, as_gray=True)

# Step 1: Threshold the image to isolate darker pixels
threshold = 0.60  # Only keep pixels darker than this value
dark_pixels_mask = image <= threshold

# Step 2: Detect contours (borders) around the cloud
contours = measure.find_contours(dark_pixels_mask, level=0.5)
...
connected_contours = [contour for contour in contours if len(contour) > 0]

endpoints = [(contour[0], contour[-1]) for contour in connected_contours]

# Ensure contours are concatenated only if they have matching dimensions
for i, (start1, end1) in enumerate(endpoints):
    for j, (start2, end2) in enumerate(endpoints):
        ...
        if min(distances) < connection_threshold:
            # Ensure contours have compatible dimensions before concatenation
            if connected_contours[i].shape[1] == connected_contours[j].shape[1]:
                ...
                connected_contours[j] = np.array([])  # Mark this contour as processed
            else:
                print(f"Dimension mismatch between contours {i} and {j}. Skipping connection.")
...
# Step 7: Map skeleton points to shapes
...
for idx, contour in connected_shapes_dict.items():
    path = Path(contour[:, [1, 0]])  # (x, y) format
    for i, point in enumerate(skeleton_points):
        if path.contains_point((point[1], point[0])):
            skeleton_point_labels[i] = idx
...
for shape_id in range(len(connected_shapes_dict)):
    ...
    if len(shape_points) < 2:
        continue
    polyline = [shape_points[0]]# Import necessary libraries
import numpy as np
import matplotlib.pyplot as plt
from skimage import io, measure
from skimage.morphology import skeletonize
...
epsilon = 1.0
optimized_polylines = {shape_id: douglas_peucker(polyline, epsilon) for shape_id, polyline in polylines.items()}

# Plot only the optimized polylines on an enlarged canvas
plt.figure(figsize=(20, 20))
for shape_id, optimized_polyline in optimized_polylines.items():
    plt.plot(
        optimized_polyline[:, 1],  # y-coordinate
        optimized_polyline[:, 0],  # x-coordinate
        '-',
        linewidth=2,
