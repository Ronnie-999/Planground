...
/**
 * Planground – raw‑sketch canvas with pen & eraser tools.
 * Eliminated duplicate "Playground" heading and legacy smoothness UI.
 */
export default function Planground() {
  ...
  // ===== initialise canvas & page metadata =====
  useEffect(() => {
    // Resize & frame
    ...
    // Remove any legacy heading that still says "Playground"
    document.querySelectorAll("h1").forEach((el) => {
      if (el.textContent.trim() === "Playground") el.remove();
    });
  }, []);

  // draw decorative frame once per clear
  ...
  const stopDrawing = (e) => {
    ...
    // restore normal mode and redraw frame if eraser trimmed it
    if (tool === "eraser") {
      ctx.globalCompositeOperation = "source-over";
